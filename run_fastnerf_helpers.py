...
class FN1(nn.Module):
    def __init__(self, D=8, W=384, input_ch=3, output_ch=25, skips=[4], use_viewdirs=False):
        """
        input xyz , output 8 dims of rgb
        """
        super(FN1, self).__init__()
        self.D = D
        self.W = W
        self.input_ch = input_ch
        self.skips = skips
        ...
        self.pts_linears = nn.ModuleList(
            [DenseLayer(input_ch, W, activation="relu")] + [
                DenseLayer(W, W, activation="relu") if i not in self.skips else DenseLayer(W + input_ch, W,activation="relu") for i in range(D - 1)])
        ...
        if use_viewdirs:
            self.feature_linear = DenseLayer(W, W, activation="linear")
            self.alpha_linear = DenseLayer(W, 1, activation="linear")
            self.rgb_linear = DenseLayer(W, 24, activation="linear")
        else:
            self.output_linear = DenseLayer(W, output_ch, activation="linear")

    def forward(self, x):
        input_pts = torch.split(x, self.input_ch, dim=-1)
        h = input_pts
        for i, l in enumerate(self.pts_linears):
            h = self.pts_linears[i](h)
            h = F.relu(h)
